-- ═══════════════════════════════════════════════════════════════════════════
-- GyfTR Legal Portal — Complete Seed Script
-- Run in: Supabase Dashboard → SQL Editor → New Query → Run
-- ═══════════════════════════════════════════════════════════════════════════

-- ── STEP 1: Fix missing RLS policies ──────────────────────────────────────
-- Profiles had no read/write policies — this broke agreement creation

DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename='profiles' AND policyname='Read profiles'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename='profiles' AND policyname='Insert own profile'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename='profiles' AND policyname='Update own profile'
  ) THEN
    EXECUTE 'CREATE POLICY "Update own profile" ON profiles FOR UPDATE USING (auth.uid() = id)';
  END IF;
END $$;

-- Also allow legal users to update agreements (agreements UPDATE policy only uses legal role check)
-- Add delete policy for legal role on agreements (cleanup)
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename='agreements' AND policyname='Legal deletes agreements'
  ) THEN
...
-- ── STEP 2: Enable pgcrypto for password hashing ───────────────────────────
CREATE EXTENSION IF NOT EXISTS pgcrypto;

-- ── STEP 3: Create test auth users ────────────────────────────────────────
-- Fixed UUIDs for reproducibility (safe to re-run — uses ON CONFLICT DO NOTHING)
DO $$
DECLARE
  uid_legal     UUID := 'aaaa0001-0000-0000-0000-000000000000';
  uid_finance    UUID := 'aaaa0002-0000-0000-0000-000000000000';
  uid_business    UUID := 'aaaa0003-0000-0000-0000-000000000000';
  uid_compliance    UUID := 'aaaa0004-0000-0000-0000-000000000000';
BEGIN
  -- Insert users into auth.users (bypasses RLS as postgres superuser)
  INSERT INTO auth.users (
    ...
    (uid_legal,  '00000000-0000-0000-0000-000000000000', 'authenticated', 'authenticated',
     'alex.carter@example.com',  crypt('ChangeMe123!', gen_salt('bf')), NOW(), NOW(),
     '{"provider":"email","providers":["email"]}', '{"name":"Alex Carter"}',
     NOW(), NOW(), '', '', '', ''),
    (uid_finance, '00000000-0000-0000-0000-000000000000', 'authenticated', 'authenticated',
     'jordan.lee@example.com', crypt('ChangeMe123!', gen_salt('bf')), NOW(), NOW(),
     '{"provider":"email","providers":["email"]}', '{"name":"Jordan Lee"}',
     NOW(), NOW(), '', '', '', ''),
    (uid_business, '00000000-0000-0000-0000-000000000000', 'authenticated', 'authenticated',
     'sam.rivera@example.com', crypt('ChangeMe123!', gen_salt('bf')), NOW(), NOW(),
     '{"provider":"email","providers":["email"]}', '{"name":"Sam Rivera"}',
     NOW(), NOW(), '', '', '', ''),
    (uid_compliance, '00000000-0000-0000-0000-000000000000', 'authenticated', 'authenticated',
     'riley.quinn@example.com', crypt('ChangeMe123!', gen_salt('bf')), NOW(), NOW(),
     '{"provider":"email","providers":["email"]}', '{"name":"Riley Quinn"}',
     NOW(), NOW(), '', '', '', '')
  ON CONFLICT (id) DO NOTHING;

  -- Insert identity records (required for email/password login)
  INSERT INTO auth.identities (id, user_id, identity_data, provider, last_sign_in_at, created_at, updated_at)
  VALUES
    (uid_legal::text,  uid_legal,  jsonb_build_object('sub', uid_legal::text,  'email', 'alex.carter@example.com'),  'email', NOW(), NOW(), NOW()),
    (uid_finance::text, uid_finance, jsonb_build_object('sub', uid_finance::text, 'email', 'jordan.lee@example.com'), 'email', NOW(), NOW(), NOW()),
    (uid_business::text, uid_business, jsonb_build_object('sub', uid_business::text, 'email', 'sam.rivera@example.com'), 'email', NOW(), NOW(), NOW()),
    (uid_compliance::text, uid_compliance, jsonb_build_object('sub', uid_compliance::text, 'email', 'riley.quinn@example.com'), 'email', NOW(), NOW(), NOW())
  ON CONFLICT (id) DO NOTHING;

  -- Insert profiles
  INSERT INTO profiles (id, name, role, team_code, avatar) VALUES
    (uid_legal,  'Alex Carter',  'legal',      'L', 'NK'),
    (uid_finance, 'Jordan Lee', 'finance',    'F', 'JL'),
    (uid_business, 'Sam Rivera', 'business',   'B', 'SR'),
    (uid_compliance, 'Riley Quinn', 'compliance', 'C', 'NK')
  ON CONFLICT (id) DO NOTHING;
END $$;

-- ── STEP 4: Seed 6 agreements ──────────────────────────────────────────────
DO $$
DECLARE
  uid_legal  UUID := 'aaaa0001-0000-0000-0000-000000000000';
  ...
  -- clause UUIDs for ag1 (Meridian Finance E2E)
  ...
  -- clause UUIDs for ag2 (Meridian Finance API)
...
INSERT INTO agreements (
  ...
  -- 1. Meridian Finance E2E (Buy & Sell)
  (ag1, 'Meridian Finance Limited', 'MFL', 'API / Direct', 'review', 'responded',
   '2026-07-01', '2026-01-10', 'Alex Carter', 'Jordan Lee', 'Sam Rivera', 'Riley Quinn',
  ...
  -- 2. Meridian Finance API Integration
  (ag2, 'Meridian Finance — API', 'MFA', 'API / Direct', 'review', 'negotiating',
   '2026-06-30', '2026-03-05', 'Alex Carter', 'Jordan Lee', 'Sam Rivera', 'Riley Quinn',
  ...
  -- 3. Ironclad White Label
  (ag3, 'Ironclad Industries', 'IRON', 'White Label', 'pending', 'awaiting',
   '2026-07-15', '2026-04-01', 'Alex Carter', 'Jordan Lee', 'Sam Rivera', 'Riley Quinn',
  ...
  -- 4. Summit Life Insurance
  (ag4, 'Summit Life Insurance', 'SLI', 'API / Direct', 'review', 'responded',
   '2026-07-01', '2026-05-12', 'Alex Carter', 'Jordan Lee', 'Sam Rivera', 'Riley Quinn',
   'https://docs.google.com/document/d/1mtLDflMAwkKM-RGZ1pABfJEpS1GcWAqW/edit',
  ...
  -- 5. Crestview Bank (Closed)
  (ag5, 'Crestview Bank', 'CVB', 'API / Direct', 'closed', 'finalised',
   '2026-04-01', '2026-01-05', 'Alex Carter', 'Jordan Lee', 'Sam Rivera', 'Riley Quinn',
   'https://docs.google.com/document/d/1mtLDflMAwkKM-RGZ1pABfJEpS1GcWAqW/edit',
  ...
  (ag6, 'Vantage Bank', 'VTB', 'Reseller', 'reopen', 'responded',
   '2026-05-30', '2026-02-01', 'Alex Carter', 'Jordan Lee', 'Sam Rivera', 'Riley Quinn',
   'https://docs.google.com/document/d/1mtLDflMAwkKM-RGZ1pABfJEpS1GcWAqW/edit',
...
INSERT INTO team_statuses (agreement_id, team_code, status, aging_days, updated_by, updated_at) VALUES
  -- AG1: Meridian Finance E2E
  (ag1,'L','Approved',0,'Alex Carter','2026-05-15 10:00:00+05:30'),
  (ag1,'F','Under Review',3,'Jordan Lee','2026-06-10 11:00:00+05:30'),
  (ag1,'C','Pending',0,'—','2026-01-10 10:00:00+05:30'),
  (ag1,'B','Approved',0,'Sam Rivera','2026-05-18 09:00:00+05:30'),
  -- AG2: Meridian Finance API
  (ag2,'L','Under Review',2,'Alex Carter','2026-06-08 10:30:00+05:30'),
  (ag2,'F','Under Review',1,'Jordan Lee','2026-06-08 10:30:00+05:30'),
  (ag2,'C','Pending',0,'—','2026-03-05 09:00:00+05:30'),
  (ag2,'B','Approved',0,'Sam Rivera','2026-04-02 10:00:00+05:30'),
  -- AG3: Ironclad
  (ag3,'L','Under Review',1,'Alex Carter','2026-06-01 14:00:00+05:30'),
  ...
  -- AG4: Summit Life
  (ag4,'L','Approved',0,'Alex Carter','2026-06-01 09:00:00+05:30'),
  (ag4,'F','Under Review',3,'Jordan Lee','2026-06-05 14:22:00+05:30'),
  (ag4,'C','Pending',0,'—','2026-05-12 11:30:00+05:30'),
  (ag4,'B','Approved',0,'Sam Rivera','2026-06-03 10:05:00+05:30'),
  -- AG5: Crestview Bank (all approved/closed)
  (ag5,'L','Approved',0,'Alex Carter','2026-02-10 10:00:00+05:30'),
  (ag5,'F','Approved',0,'Jordan Lee','2026-03-01 09:00:00+05:30'),
  (ag5,'C','Approved',0,'Riley Quinn','2026-03-10 14:00:00+05:30'),
  (ag5,'B','Approved',0,'Sam Rivera','2026-02-15 11:00:00+05:30'),
  -- AG6: Vantage Bank
  (ag6,'L','Under Review',3,'Alex Carter','2026-06-07 10:30:00+05:30'),
  (ag6,'F','Rejected',5,'Jordan Lee','2026-06-06 15:00:00+05:30'),
  (ag6,'C','Pending',0,'—','2026-02-01 11:00:00+05:30'),
  (ag6,'B','Approved',0,'Sam Rivera','2026-03-10 09:00:00+05:30')
...
INSERT INTO remarks (agreement_id, author_name, author_role, text, created_at) VALUES
  -- AG1
  (ag1,'Alex Carter','Legal','D1 sent — standard E2E Buy & Sell template used. Revenue share at 15% as per business approval.','2026-05-15 10:00:00+05:30'),
  (ag1,'Sam Rivera','Business','Meridian confirmed interest. Client wants payment cycle changed from 30 to 45 days.','2026-05-20 14:30:00+05:30'),
  (ag1,'Jordan Lee','Finance','45-day payment cycle acceptable but need prefunded advance account clause confirmed. Awaiting Legal to revise Annexure A.','2026-06-10 11:00:00+05:30'),
  -- AG2
  (ag2,'Alex Carter','Legal','API template sent. Key additions vs E2E: Proprietary Software clause, API uptime SLA, data security obligations.','2026-03-10 09:00:00+05:30'),
  (ag2,'Sam Rivera','Business','Meridian happy with API access. Dispute on uptime SLA — they want 99.9%, we offered 99.5%.','2026-04-05 15:00:00+05:30'),
  (ag2,'Jordan Lee','Finance','Revenue share model needs clarification. Is it on MRP or selling price? Need Legal to specify in Annexure A clearly.','2026-06-08 10:30:00+05:30'),
  -- AG3
  (ag3,'Alex Carter','Legal','WL template sent. This is more complex — includes WhiteLabel website development, payment gateway, API, and direct sending module. All 4 fulfilment methods included.','2026-04-05 09:00:00+05:30'),
  (ag3,'Sam Rivera','Business','Ironclad is for channel incentivization. Large deal — ~Rs 2Cr annual GMV expected. Client wants custom branding on WL site.','2026-04-10 11:00:00+05:30'),
  (ag3,'Alex Carter','Legal','D1 sent. Awaiting client response. Remind in 7 days if no reply.','2026-06-01 14:00:00+05:30'),
  -- AG4
  (ag4,'Alex Carter','Legal','Draft shared with finance team for review.','2026-06-01 09:00:00+05:30'),
  (ag4,'Jordan Lee','Finance','Awaiting clarification on revenue share clause — is the 17% on MRP or selling price?','2026-06-05 14:22:00+05:30'),
  -- AG5
  (ag5,'Alex Carter','Legal','Fully executed. Signed copies filed. Agreement live from 1 May 2026.','2026-04-10 12:00:00+05:30'),
  -- AG6
  (ag6,'Jordan Lee','Finance','Rejected. Revenue share of 17% is below our minimum threshold of 18% for resellers. Please rework Annexure A and resubmit.','2026-06-06 15:00:00+05:30'),
  (ag6,'Alex Carter','Legal','Noted. Will rework clauses 5 and Annexure A. New draft by June 15.','2026-06-07 10:30:00+05:30')
...
INSERT INTO drafts (agreement_id, draft_no, direction, note, date, created_by) VALUES
  ...
  (ag1,'D2','received','Meridian returned with 4 changes: payment cycle 45 days, liability cap increase, non-solicitation period reduced to 6 months, added KYC clause.','2026-02-01',uid_legal),
  ...
  (ag1,'D4','received','Meridian accepted D3 terms. Minor edit on escalation matrix contact details only.','2026-06-04',uid_legal),
  -- AG2
  ...
  -- AG4
  (ag4,'D1','sent','Initial draft sent to Summit Life. Standard API template. Revenue share 17%.','2026-05-14',uid_legal),
  ...
  (ag4,'D4','received','Summit Life accepted most terms. Still pushing on SLA — want 99.7%.','2026-06-04',uid_legal),
  -- AG5
  ...
  (ag6,'D2','received','Vantage proposed major revision: revenue share 12%, exclusivity in banking channel, 2-year term.','2026-03-01',uid_legal),
  (ag6,'D3','sent','GyfTR counter: revenue share 17% (firm), no exclusivity, 18-month term.','2026-04-01',uid_legal),
  (ag6,'D4','received','Vantage accepted 17% and 18-month term. Withdrew exclusivity request.','2026-05-15',uid_legal),
...
INSERT INTO history_log (agreement_id, team, changed_by, from_status, to_status, created_at) VALUES
  -- AG1
  (ag1,'Legal','Alex Carter','—','Pending','2026-01-10 10:00:00+05:30'),
  (ag1,'Legal','Alex Carter','Pending','Approved','2026-05-15 10:00:00+05:30'),
  (ag1,'Business','Sam Rivera','Pending','Approved','2026-05-18 09:00:00+05:30'),
  (ag1,'Finance','Jordan Lee','Pending','Under Review','2026-06-10 11:00:00+05:30'),
  -- AG2
  (ag2,'Legal','Alex Carter','—','Pending','2026-03-05 09:00:00+05:30'),
  (ag2,'Legal','Alex Carter','Pending','Under Review','2026-03-10 09:00:00+05:30'),
  (ag2,'Business','Sam Rivera','Pending','Approved','2026-04-02 10:00:00+05:30'),
  (ag2,'Finance','Jordan Lee','Pending','Under Review','2026-06-08 10:30:00+05:30'),
  -- AG3
  (ag3,'Legal','Alex Carter','—','Pending','2026-04-01 09:00:00+05:30'),
  (ag3,'Legal','Alex Carter','Pending','Under Review','2026-06-01 14:00:00+05:30'),
  -- AG4
  (ag4,'Legal','Alex Carter','—','Pending','2026-05-12 11:30:00+05:30'),
  (ag4,'Legal','Alex Carter','Pending','Approved','2026-06-01 09:00:00+05:30'),
  (ag4,'Business','Sam Rivera','Pending','Approved','2026-06-03 10:05:00+05:30'),
  (ag4,'Finance','Jordan Lee','Pending','Under Review','2026-06-05 14:22:00+05:30'),
  -- AG5
  (ag5,'Legal','Alex Carter','—','Pending','2026-01-05 09:00:00+05:30'),
  (ag5,'Legal','Alex Carter','Pending','Approved','2026-02-10 10:00:00+05:30'),
  (ag5,'Business','Sam Rivera','Pending','Approved','2026-02-15 11:00:00+05:30'),
  (ag5,'Finance','Jordan Lee','Under Review','Approved','2026-03-01 09:00:00+05:30'),
  (ag5,'Compliance','Riley Quinn','Under Review','Approved','2026-03-10 14:00:00+05:30'),
  (ag5,'Legal','Alex Carter','Final Sign','Closed','2026-04-10 12:00:00+05:30'),
  -- AG6
  (ag6,'Legal','Alex Carter','—','Pending','2026-02-01 11:00:00+05:30'),
  (ag6,'Business','Sam Rivera','Pending','Approved','2026-03-10 09:00:00+05:30'),
  (ag6,'Finance','Jordan Lee','Under Review','Approved','2026-04-20 10:00:00+05:30'),
  (ag6,'Finance','Jordan Lee','Approved','Rejected','2026-06-06 15:00:00+05:30')
ON CONFLICT DO NOTHING;

-- ── CLAUSES — AG1 (Meridian Finance E2E) ──────────────────────────────────────
INSERT INTO clauses (id, agreement_id, clause_no, clause_name, outcome, full_context) VALUES
  (cl1a, ag1, '2', 'Scope of Work — Fulfilment Method', 'accepted',
   'Meridian requested addition of API fulfilment alongside Excel file delivery. GyfTR agreed to include API as an optional fulfilment channel subject to technical integration.'),
  (cl1b, ag1, '5', 'Consideration — Payment Cycle', 'partial',
   'Initial template had 30-day payment cycle. Meridian cited internal finance processing time and requested 45 days. GyfTR accepted subject to prefunded advance account being maintained.'),
  (cl1c, ag1, '11', 'Non-Solicitation and Anti-Poaching', 'held',
   'Meridian proposed reducing the non-solicitation period from 12 months to 6 months post-termination. GyfTR held firm at 12 months citing merchant relationship protection.'),
  (cl1d, ag1, '13', 'Indemnity — Liability Cap', 'partial',
   'Template had liability cap at 3 months fees. Meridian requested increase to 6 months. Settled at 4 months as a compromise.')
...
-- ── CLAUSES — AG2 (Meridian Finance API) ──────────────────────────────────────
INSERT INTO clauses (id, agreement_id, clause_no, clause_name, outcome, full_context) VALUES
  ...
  (cl2d, ag2, '11', 'Non-Solicitation', 'held',
   'Client attempted to remove non-solicitation clause entirely, arguing it restricts their ability to work with merchants independently. GyfTR held firm — this clause is in every agreement.'),
  (cl2e, ag2, '13', 'Indemnity — Data Breach Carve-out', 'pending',
   'Client wants data breach liability carved out from the overall 3-month fee cap, arguing data breach exposure is unlimited under IT Act. GyfTR legal reviewing with compliance. Still unresolved.')
ON CONFLICT (agreement_id, clause_no) DO NOTHING;
...
-- ── CLAUSES — AG4 (Summit Life Insurance) ───────────────────────────────────
DO $$
DECLARE
...
BEGIN
  INSERT INTO clauses (id, agreement_id, clause_no, clause_name, outcome, full_context) VALUES
    ...
    (cl4b, ag4, '5', 'Payment Cycle', 'accepted',
     'Template was 30 days. Summit Life requested 45 days citing internal finance processing. Accepted.'),
    (cl4c, ag4, '6', 'API Uptime SLA', 'pending',
     'Summit Life wants 99.7% SLA. GyfTR currently offering 99.5%. Gap remains. Finance also needs to approve the penalty structure before Legal can agree.'),
...
BEGIN
  ...
  INSERT INTO clause_changes (clause_id, draft_no, change_text) VALUES
    (cl5a,'D1','30 days'),(cl5a,'D2','Crestview requested 45 days'),(cl5a,'D3','Accepted'),(cl5a,'D4','Final'),
    (cl5b,'D1','3 months fees'),(cl5b,'D2','Crestview requested 6 months'),(cl5b,'D3','Counter at 4 months'),(cl5b,'D4','Agreed 4 months')
  ON CONFLICT (clause_id, draft_no) DO NOTHING;
END $$;

-- ── CLAUSES — AG6 (Vantage Bank) ─────────────────────────────────────────────
DO $$
DECLARE
...
BEGIN
  INSERT INTO clauses (id, agreement_id, clause_no, clause_name, outcome, full_context) VALUES
    ...
    (cl6b, ag6, '11', 'Non-Solicitation', 'held',
     'Vantage attempted to remove non-solicitation clause citing it restricts their reseller model. GyfTR held firm.'),
    (cl6c, ag6, '12', 'Term', 'partial',
     'Template 12 months. Vantage wanted 24 months. Settled at 18 months.')
  ON CONFLICT (agreement_id, clause_no) DO NOTHING;

  INSERT INTO clause_changes (clause_id, draft_no, change_text) VALUES
    (cl6a,'D1','GyfTR revenue share: 18% of net GMV (standard reseller rate).'),
    (cl6a,'D2','Vantage proposed 12% citing thin reseller margin.'),
    (cl6a,'D3','GyfTR counter: 17% flat — accepted by Business team without Finance approval.'),
    (cl6a,'D4','Vantage accepted 17%. Sent for internal GyfTR approval.'),
...
-- ═══════════════════════════════════════════════════════════════════════════
-- DONE. Verify with:
-- SELECT count(*) FROM agreements;     -- should be 6
-- SELECT count(*) FROM team_statuses;  -- should be 24
-- SELECT count(*) FROM remarks;        -- should be 14
-- SELECT count(*) FROM drafts;         -- should be 23
-- SELECT count(*) FROM clauses;        -- should be 18
-- SELECT count(*) FROM clause_changes; -- should be 68
-- SELECT count(*) FROM history_log;    -- should be 24
-- SELECT id, email FROM auth.users WHERE email LIKE '%gyftr%';  -- should be 4
-- ═══════════════════════════════════════════════════════════════════════════
