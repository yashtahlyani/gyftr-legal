...
5. ~~Reminders/"nudges" are not persisted~~ **RESOLVED.** `sendNudge` still writes to the in-memory `reminderLog` too (that's what still drives demo mode, which has no backend at all), but real sessions now hydrate the notification bar from `GET /api/reminders` on login and persist dismissals via `PATCH /api/reminders/:id/dismiss` — see `_loadReminders()` / `dismissReminderBar()` in `app-logic.js`. A reminder sent from one person's browser now actually reaches the recipient's, on a different machine, after a refresh.
...
3. Decide what to do with the remaining known duplication (§6.5 items 1, 2, 3, 7 below — items 4, 5, and 6 are now resolved, see `infra/HANDOVER.md` "Decisions made during the migration"): the sample data/formatting-helper duplication between `sample.js`/`utils.js` and their inline copies in `app-logic.js` is still there and untouched.
...
5. ~~Decide whether reminders/nudges and the Drafts modal should be made persistent~~ **RESOLVED** — both now have a working backend and the frontend actually calls it: reminders hydrate/dismiss for real (§6.5 item 5), and the Drafts modal has a real file-upload control wired to the same S3 route the backend already had.
